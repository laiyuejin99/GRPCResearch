...
internal fun <S : MessageLite> newRequestBody(onlyMessage: S): RequestBody {
    return object : RequestBody() {
        override fun contentType() = APPLICATION_GRPC_MEDIA_TYPE

        override fun writeTo(sink: BufferedSink) {
            GrpcMessageSink<S>(sink, grpcEncoding = "identity").use {
...
internal fun <S : MessageLite> PipeDuplexRequestBody.messageSink(): GrpcMessageSink<S> {
    return GrpcMessageSink(createSink(), grpcEncoding = "identity")
}

/** Sends the response messages to the channel. */
internal fun <R : MessageLite> SendChannel<R>.readFromResponseBodyCallback(responseClass: Class<R>): Callback {
 ...
 */
internal fun <S : MessageLite> ReceiveChannel<S>.writeToRequestBody(
    requestBody: PipeDuplexRequestBody
) {
    CoroutineScope(Dispatchers.IO).launch {
        requestBody.messageSink<S>().use { requestWriter ->
            consumeEach { message ->
...
internal fun <R : MessageLite> Response.messageSource(
    responseClass: Class<R>
): GrpcMessageSource<R> {
    val grpcEncoding = header("grpc-encoding")
    val responseSource = body!!.source()
    return GrpcMessageSource(responseSource, responseClass, grpcEncoding)
}
